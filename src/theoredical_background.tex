...
\end{toreview}
    ...
    \textbf{Scalable Oversight} addresses the problem that humans cannot reliably evaluate the outputs of a superintelligent system. \textit{Debate} trains two \gls{ai} systems to argue opposing positions so that a human judge can identify the truth from argument quality alone. \textit{Sandwiching Evaluations} test models on tasks of calibrated difficulty to gauge reliability. \textit{Weak-to-Strong Generalization} explores whether a weaker model can effectively supervise a more capable one.
...
\end{toreview}
    ...
    \textbf{Containment and Restriction} limit what the \gls{ai} can do, as a last line of defense. \textit{Boxing} or \textit{Air-gapping} physically isolates the system from the outside world, while an \textit{Oracle \gls{ai}} architecture restricts the system to answering questions without the ability to act.
