...
In the world of \gls{ai} safety, there's a lot of research surrounding preventing misbehavior in a distributional shift, explainable \gls{ai}, etc. The proposed hypothesis is the following:
\begin{itemize}
	\item \textbf{Hypothesis}: If the current trajectory of \gls{ai} development is deemed to be unsafe, then, in the present environment, where every big \gls{ai} company is incentivized to invest large swaths of money into this technology, no amount of \gls{ai} safety research is enough, meaning that to truly guarantee safe and aligned \gls{ai} systems for humans in the future, changes to the incentive structure that pushes companies in this direction are also required.
\end{itemize}
