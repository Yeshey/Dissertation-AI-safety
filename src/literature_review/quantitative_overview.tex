\begin{reviewed}
    ...
    Table~\ref{tab:severity_coverage} compares severity scores with the number of papers assigned to each category.
	...
	\centering
	\scriptsize
	\caption{Category coverage against a severity-weighted expectation. Each severity point corresponds to $143/24.75=5.78$ papers, so Expected is the category's severity multiplied by that rate. Actual $-$ Exp.\ is Papers minus Expected; Coverage is Papers divided by Expected, where $1.00$ means the category has exactly the coverage its severity predicts.}
	\label{tab:severity_coverage}
	\resizebox{\textwidth}{!}{%
	\begin{tabular}{lrrrrr}
	\toprule
	Category & Severity & Papers & Expected & Actual $-$ Exp. & Coverage \\
	\midrule
	Human \& Societal Impacts    & 4.10 & 35 & 23.7 & $+11.3$ & 1.48 \\
	Governance, Reg.\ \& Policy   & 3.90 & 28 & 22.5 & $+5.5$  & 1.24 \\
	Alignment                     & 3.70 & 30 & 21.4 & $+8.6$  & 1.40 \\
	Auditing, Safety \& Oversight & 3.55 & 21 & 20.5 & $+0.5$  & 1.02 \\
	Bias \& Fairness              & 3.40 & 9  & 19.6 & $-10.6$ & 0.46 \\
	Liability \& Ethics           & 3.05 & 8  & 17.6 & $-9.6$  & 0.45 \\
	Explainability / Interp.      & 3.05 & 12 & 17.6 & $-5.6$  & 0.68 \\
	\bottomrule
	\end{tabular}
	}
	\end{table}

	Severity and coverage do not align perfectly. \emph{Bias \& Fairness} is the clearest case: it scores higher in severity (3.40) than either \emph{Explainability / Interpretability} or \emph{Liability \& Ethics} (both 3.05), yet has fewer papers (9) than Explainability (12) and only one more than Liability (8). All three fall below what their severity predicts, with coverage ratios of 0.46, 0.45, and 0.68 respectively. At the other end, \emph{Governance, Regulation \& Policy} ranks second in severity but has fewer papers than \emph{Alignment}, though both exceed their expected counts. \emph{Auditing, Safety \& Oversight} lands almost exactly on its expectation. Overall, research attention broadly follows assessed severity, but some categories receive relatively more or less attention than their severity would suggest.
