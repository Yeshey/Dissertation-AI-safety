...
\begin{toreview}
	This chapter reports the findings of the \gls{slr}, which was conducted to address the four research questions defined in Section~\ref{chap:introduction} (\gls{rq}1--\gls{rq}4). The primary objective of this review is to map the current AI safety landscape and pinpoint critical gaps in the literature. Ultimately, identifying which specific alignment issue requires urgent empirical testing, motivating the experiments conducted in Chapter~\ref{chap:empirical_evaluation}.
\end{toreview}
	...
	\begin{itemize}
		...
		\item \textbf{Datasets:} The search engines used are two brokers: Scopus\footnote{https://www.scopus.com} and Web of Science\footnote{https://apps.webofknowledge.com}, in conjunction with IEEE\footnote{https://ieeexplore.ieee.org}, ACM\footnote{https://dl.acm.org} and EBSCO\footnote{https://search.ebscohost.com}.
	\end{itemize}

	Because the \gls{ai} field is evolving at a pace that is sometimes hard for peer-reviewed literature to keep up with, snowballing is introduced to bring in other relevant articles and gray literature \cite{wohlin_guidelines_2014}. The related table can be found in Appendix \ref{sec:snowballing_articles_slr}.

	A systemic search for literature in all the databases is conducted, applying ever more strict filters to narrow the search to only the most relevant articles. The progress made in this phase is documented in Table \ref{tab:slr1_filters}, and the inclusion and exclusion criteria across filters are clearly defined in Table \ref{tab:inclusion_exclusion_criteria}. 
	...
	A noteworthy consideration is the growth that the \gls{ai} safety field has experienced in the past couple of years. Since the advent of \glspl{llm} that \gls{ai} has been a booming topic; this can be verified in the volume of publications, as shown in Figure \ref{fig:ai_trends_combined}, or in the volume of web searches like shown in Figure \ref{fig:google_trends_ai_safety}.

	In much the same way, \gls{ai} safety publications have also enjoyed an increase in output, which is also reflected in Figure \ref{fig:ai_trends_combined}, especially with the release of ChatGPT on November 30, 2022. Albeit with fewer advancements, largely due to financial incentives \cite{eto_state_global_ai_research_2024, ahmed_narrow_depth_breadth_corporate_2024, hemphill_advanced_2025}. The same can be verified in general web searches, Figure \ref{fig:google_trends_ai_safety}.

	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.75\textwidth]{./src/imgs/Combined_AI_timeGraph_Scopus}
		\caption{Number of documents mentioning \gls{ai} (bottom) and mentioning \gls{ai} and containing "\gls{ai} safety" (top) over time (Scopus).}
		\label{fig:ai_trends_combined}
	\end{figure}
...
\FloatBarrier
\input{src/literature_review/research_results/research_results.tex}
