...
The systematic literature search was executed using target databases and snowballing techniques to capture recent developments. The intended literature was captured by:
	...
	Because the \gls{ai} field is evolving at a pace that is sometimes hard for peer-reviewed literature to keep up with, snowballing is introduced to bring in other relevant articles and gray literature \cite{wohlin_guidelines_2014}. The related table can be found in Appendix \ref{sec:snowballing_articles_slr}.
		...
		A systematic search for literature in all the databases is conducted, applying ever stricter filters to narrow the search to only the most relevant articles. The progress made in this phase is documented in Table \ref{tab:slr1_filters}, and the inclusion and exclusion criteria across filters are clearly defined in Table \ref{tab:inclusion_exclusion_criteria}. 
	...
	\begin{table}[htbp]
		...
		\begin{threeparttable}
			...
			\begin{tablenotes}
				...
				\item Filter 5 = After Abstract Screening and Concept Matrix
				\item Snowballing = Full-text Document Assessment
			\end{tablenotes}
		\end{threeparttable}
	\end{table}
	...
	\end{table}
		...
		A noteworthy consideration is the growth that the \gls{ai} safety field has experienced in the past couple of years. Since the advent of \glspl{llm}, \gls{ai} has been a booming topic; this can be verified in the volume of publications, as shown in Figure \ref{fig:ai_trends_combined}, or in the volume of web searches, as shown in Figure \ref{fig:google_trends_ai_safety}.

		The rate of growth of \gls{ai} safety publications is relatively low, with few advancements in the field, largely due to the lack of financial incentives \cite{eto_state_global_ai_research_2024, ahmed_narrow_depth_breadth_corporate_2024, hemphill_advanced_2025}. This trend can also be verified in general web searches (Figure \ref{fig:google_trends_ai_safety}).
	...
	\begin{figure}[htbp]
		...
		\caption{Volume of Web Searches for the terms ``AI Safety'' and ``Artificial Intelligence''}
