...
\textbf{\glspl{llm}} are neural networks trained to predict the next token in a sequence, optimized over vast corpora of text until the predicted distribution closely matches human-written language. In their base form they have no notion of instruction-following or dialogue; they simply continue text. To produce the conversational, instruction-following models used in practice, base models undergo Supervised Fine-Tuning (SFT) on curated examples of human--assistant exchanges, which teaches the model to emulate an assistant in a conversational format. The resulting model can follow instructions but remains prone to harmful, unhelpful or sexually explicit outputs wherever such continuations are deemed statistically plausible.

\textbf{\gls{rlhf}} is the standard post-training pipeline used to suppress those residual failures. Human annotators rate model outputs, penalizing harmful, dishonest, or unhelpful responses, and these ratings train a separate \textit{reward model} that learns to predict human preferences. The \gls{llm} is then fine-tuned via reinforcement learning to maximize the reward model's score, subject to a constraint that keeps its outputs close to the SFT checkpoint. This prevents the model from drifting into incoherence while chasing reward. The pipeline is effective at producing safe and helpful behavior in distribution.
...
\textbf{Quantization} is a model compression technique that reduces the numerical precision of a model's weights from the standard 16-bit or 32-bit floating point to lower bit-widths such as 8-bit integers or 4-bit representations. This reduces memory footprint and inference cost at the expense of a small accuracy degradation. All models in the empirical evaluation are served in 4-bit quantized \gls{gguf} format, which makes it possible to run models up to 17B parameters on consumer-grade \glspl{gpu} with 11--48\,GB of \gls{vram}.

\textbf{GGUF} is a binary file format for storing (typically quantized) model weights, developed by the \textit{llama.cpp} project. It packages weights, tokenizer data, and metadata into a single file optimized for fast loading and low-overhead inference on \gls{cpu} and consumer \gls{gpu} hardware, and is the format used for all models served in this work's pipeline.

\textbf{Fisher's Exact Test} is a statistical test that checks whether the difference between two values is large enough to be meaningful, or small enough to fall within normal statistical expectancy. This makes it well-suited to small sample sizes, which is why it serves as the primary statistical measure for compliance-gap significance in Chapter~\ref{chap:empirical_evaluation}.
