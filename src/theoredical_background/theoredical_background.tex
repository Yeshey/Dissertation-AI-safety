\chapter{Theoretical Background}\label{chap:theoredical_background}

\begin{reviewed}

	\gls{ai} safety is a very broad term that is fuzzily defined; this section will explore the history of the field and compartmentalize it into several digestible areas. This is a theoretical foundation that will look into all the most common \gls{ai} safety terms and terminology that will be looked into across this work.
	...
	The evolution of the field can be roughly divided into three distinct eras: A Speculative Era (Pre-2010), the move towards Engineering Solutions (2010 -- 2014), and Mainstream Attention and Existential Risk (2014 -- Present). The timeline is illustrated in Figure \ref{fig:ai_safety_timeline}.

	\input{src/theoredical_background/timeline.tex}

	\subsection*{Speculative Era}

	Risks from \gls{ai} were discussed early on, with Norbert Wiener warning of machine defiance in 1949 \cite{wiener1950}. Before 2010, the field remained largely theoretical, focusing on philosophy and machine ethics \cite{whitby1988}. An early institutional milestone was the founding of the Singularity Institute for Artificial Intelligence (later renamed \gls{miri}) in 2001 \cite{yudkowsky2001friendly}, one of the first organizations dedicated exclusively to researching safe and beneficial AI. A critical milestone was Steve Omohundro's 2008 paper on "Basic AI Drives" \cite{omohundro2008basic}. This established the theoretical roots of Instrumental Convergence (see Section \ref{sec:foundations}), arguing that optimizing agents naturally develop dangerous sub-goals, such as resource acquisition, regardless of their initial programming.

	\subsection*{Move towards Engineering Solutions}

	A pivotal shift occurred in 2011 when Roman Yampolskiy introduced the term "AI safety engineering" \cite{yampolskiy2011, yampolskiy_controllability_2022}. The field transitioned from asking "What should an AI want?", to building safe, controllable systems using methodologies from cybersecurity and control theory \cite{control_theory_safety}. This engineering mindset was solidified by the 2016 publication of \textit{Concrete Problems in AI Safety} \cite{amodei2016concrete}, an influential agenda that formally outlined technical issues like reward hacking and safe exploration. DeepMind further formalized the field in 2018 by proposing a taxonomy of three technical safety pillars: Specification, Robustness, and Assurance. This became a canonical reference for safety engineering research \cite{ortega2018building}.

	\subsection*{Mainstreaming and Existential Risk}

	Nick Bostrom's 2014 book \textit{Superintelligence} \cite{bostrom2014superintelligence} brought AI safety concerns, namely,  Existential Risk and the Control Problem, into mainstream discourse, prompting prominent tech figures like Elon Musk, Bill Gates, and Stephen Hawking to voice similar concerns. This drove significant funding into safety institutions. In 2015, the \gls{fli} awarded \$6.5 million in grants for beneficial \gls{ai} research. The 2017 Asilomar Conference on Beneficial AI produced 23 governance principles signed by over 1,700 AI researchers, representing the field's first broad normative consensus \cite{futureoflife2017asilomar}. As deep learning accelerated --- fueled significantly by the breakthrough introduction of the Transformer architecture in 2017 \cite{vaswani2017attention} --- experts increasingly debated the probability of these civilizational threats \cite{field_why_2025}, and Hendrycks et al. updated the technical roadmap with \textit{Unsolved Problems in ML Safety} \cite{hendrycks2021unsolved}, identifying Robustness, Monitoring, Alignment, and Systemic Safety as open research directions. In March 2023, the \gls{fli} called for a six-month moratorium on training AI systems more powerful than GPT-4, gathering over 30,000 signatures and catalyzing global debate on AI risk \cite{futureoflife2023pause}. Recently, AI safety has become a priority in global governance. This shift is evidenced by the first Global AI Safety Summit held at Bletchley Park in late 2023, a formalized bilateral partnership between the UK and the US in 2024 \cite{ukus2024mou}, and the 2025 International \gls{ai} Safety Report, an effort by the UK government to comprehensively review \gls{ai} risks \cite{AISafetyReport2025}.
...
\section{The Alignment Problem}\label{sec:alignment-problem}
	...
	algorithm \cite{hubinger2019risks} (see Figure~\ref{fig:mesa-optimisation-chain}), it may develop an internal mesa-objective that diverges from the training objective. Goal Misgeneralization is a downstream symptom, where the learned goal holds during training but breaks down under distributional shift.

	\input{src/theoredical_background/aligment_graph.tex}
...
\section{Alignment Solutions and their Problems}\label{sec:solutions}
	...
	Given the failure modes described above, a range of technical approaches have been proposed to make \gls{ai} systems safer. These can be organized by the mechanism through which they act. Table \ref{tab:solutions_mapping} maps these solution categories to the specific alignment problems they primarily aim to address.

	\begin{table}[h]
		\centering
		\caption{Mapping of alignment solutions to \gls{ai} failure modes}
		\label{tab:solutions_mapping}
		
		\renewcommand{\arraystretch}{1.3}
		\small
		\begin{tabular}{@{}p{0.24\textwidth}p{0.28\textwidth}p{0.38\textwidth}@{}}
			\hline
			\textbf{Solution Category} & \textbf{Key Techniques} & \textbf{Failure Modes Addressed} \\ \hline
			\textbf{Value Specification} & Constitutional \gls{ai},\newline \gls{rlhf} & Outer Misalignment, Behavioral Failures \\ 
			\textbf{Scalable Oversight} & Sandwiching Evaluations & Deceptive Alignment, Outer Misalignment \\ 
			\textbf{Corrigibility} & \gls{cirl},\newline Utility Indifference & Inner Misalignment, Deceptive Alignment \\ 
			\textbf{Robustness and Adversarial Training} & Adversarial Training,\newline Certified Robustness & Inner Misalignment, Behavioral Failures \\ 
			\textbf{Containment and Restriction} & \gls{ai} Control, Boxing,\newline Oracle \gls{ai} & Deceptive Alignment \\ 
			\textbf{Evaluation and Responsible Deployment} & Capability Evaluations,\newline \glspl{rsp}\textsuperscript{a} & Cross-cutting evaluation and governance \\ \hline
		\end{tabular}
		
		\vspace{0.4em}
		\noindent\footnotesize\textsuperscript{a}A framework developed and used by Anthropic to mitigate catastrophic risks from \gls{ai} systems~\cite{anthropic2026rsp}.
	\end{table}
	...
	\textbf{Evaluation and Responsible Deployment} methods provide ongoing assurance after training. Dangerous Capability Evaluations test whether a model has crossed capability thresholds that trigger heightened safety requirements \cite{FLIAISafetyIndex2025, FLIAISafetyIndex2026, AISafetyReport2025}. To formalize this, frontier AI developers adopt \textbf{\glspl{rsp}}, which are formal governance and safety frameworks defined and used by Anthropic. They explicitly define in advance which safety evaluations and capability thresholds must be passed before a more capable model may be trained further or deployed, acting as a structured safety net against catastrophic risks \cite{anthropic2026rsp}.

	\section{Bias, Fairness and \gls{ai} in Society}\label{sec:bias-fairness}

	Even a well-aligned \gls{ai} can cause harm if trained on biased data or aligned to the wrong values, it is necessary to bridge safety and ethics by asking which values to encode and for whom \cite{milliere_normative_2025}.
	...
	The Impossibility Theorem of Fairness shows that common definitions (Demographic Parity, Equalized Odds, Calibration) cannot be jointly satisfied \cite{kleinberg2016inherent}, forcing explicit value trade-offs. This is compounded by Value Pluralism: there is no unified set of human values to align to, and whose values to encode varies across cultures and contexts. Harms from misalignment fall into Allocative (denied resources in hiring, lending, bailing) and Representational (stereotype amplification) categories \cite{RajiDobbe2024}.

\section{\gls{ai} Safety Map}\label{ai_safety_map}

This section presents a visual map organizing the terminology and subfields (introduced above) into a single taxonomy (Figure~\ref{fig:ai-safety-map}). The delimiters between concepts are not absolute; many areas overlap conceptually or share methodology. An empirical analysis of relative research coverage and urgency across these fields, grounded in systematic literature review data, is presented in Section~\ref{subsec:results_quantitative}.
...
\section{Key Technical Terms}\label{sec:ai_terms_relevant}

This section defines technical terms that recur throughout the empirical chapters but fall outside the alignment-specific taxonomy.

\textbf{\glspl{llm}} are neural networks trained to predict the next token in a sequence, optimized over vast corpora of text until the predicted distribution closely matches human-written language~\cite{radford2018improving, brown2020language}. In their base form they have no notion of instruction-following or dialogue; they simply continue text. To produce the conversational, instruction-following models used in practice, base models undergo Supervised Fine-Tuning (SFT) on curated examples of human--assistant exchanges~\cite{ouyang2022training}, which teaches the model to emulate an assistant in a conversational format. The resulting model can follow instructions but remains prone to harmful, unhelpful or sexually explicit outputs wherever such continuations are deemed statistically plausible.

\textbf{\gls{rlhf}} is the standard post-training pipeline used to suppress those residual failures~\cite{ziegler2019fine, stiennon2020learning}. Human annotators rate model outputs, penalizing harmful, dishonest, or unhelpful responses, and these ratings train a separate \textit{reward model} that learns to predict human preferences~\cite{bai2022training}. The \gls{llm} is then fine-tuned via reinforcement learning~\cite{schulman2017proximal} to maximize the reward model's score, subject to a constraint that keeps its outputs close to the SFT checkpoint~\cite{ouyang2022training}. This prevents the model from drifting into incoherence while chasing reward. The pipeline is effective at producing safe and helpful behavior in distribution.

\textbf{\gls{cot}} prompting elicits intermediate reasoning steps from a model before it produces a final answer, typically by instructing it to ``think step by step'' or by providing a dedicated reasoning scratchpad. Empirically, \gls{cot} improves performance on tasks requiring multi-step reasoning \cite{wei2022cot} and has been shown to influence alignment-relevant behaviors: models reasoning explicitly through a situation produce more consistent and reflective outputs, which is why the scratchpad condition is a core variable in the experiments of Chapter~\ref{chap:empirical_evaluation}. However, \gls{cot} reasoning is not necessarily faithful; models can produce post-hoc rationalizations that do not reflect the actual computation driving their outputs \cite{yu2025implicit}, a limitation relevant to interpreting scratchpad content as evidence of genuine reasoning.

\textbf{Quantization} is a model compression technique that reduces the numerical precision of a model's weights from the standard 16-bit or 32-bit floating point to lower bit-widths such as 8-bit integers~\cite{dettmers2022llmint8} or 4-bit representations~\cite{dettmers2023qlora, frantar2023gptq}. This reduces memory footprint and inference cost at the expense of a small accuracy degradation. All models in the empirical evaluation are served in 4-bit quantized \gls{gguf} format, which makes it possible to run models up to 17B parameters on consumer-grade \glspl{gpu} with 11--48\,GB of \gls{vram}.

\textbf{GGUF} is a binary file format for storing (typically quantized) model weights, developed by the \textit{llama.cpp} project~\cite{gerganov2023llamacpp}. It packages weights, tokenizer data, and metadata into a single file optimized for fast loading and low-overhead inference on \gls{cpu} and consumer \gls{gpu} hardware, and is the format used for all models served in this work's pipeline.

\textbf{Fisher's Exact Test}~\cite{fisher1935design} is a statistical test that checks whether the difference between two values is large enough to be meaningful, or small enough to fall within normal statistical expectancy. This makes it well-suited to small sample sizes, which is why it serves as the primary statistical measure for compliance-gap significance in Chapter~\ref{chap:empirical_evaluation}.
