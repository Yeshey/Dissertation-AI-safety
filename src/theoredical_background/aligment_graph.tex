% ------------------------------------------------------------------
%  FIGURE: Mesa-Optimization Chain
% ------------------------------------------------------------------
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
...
% ----------------------------------------------------------------
%  A) HUMAN --- circle labeled "Human" with tiny stick body below
% ----------------------------------------------------------------
\node[
...
% ----------------------------------------------------------------
%  C) BASE OPTIMIZER BOX  (shifted left slightly to 5.2)
% ----------------------------------------------------------------
\node[bigbox] (baseopt) at (5.2, 2.10) {};
\node[above, font=\small\bfseries, anchor=south] at (baseopt.north)
    {Base Optimizer};

\node[objbox] (baseobj) at (5.2, 0.82)
    {(BASE)\\[-3pt]Objective};

% ----------------------------------------------------------------
%  D) MODEL (MESA-OPTIMIZER) BOX  (shifted right to 10.4)
% ----------------------------------------------------------------
\node[bigbox] (model) at (10.4, 2.10) {};
\node[above, align=center, font=\small\bfseries, anchor=south] at (model.north)
    {Model\\[-3pt]\normalfont\scriptsize(Mesa-Optimizer)};

\node[objbox] (mesaobj) at (10.4, 0.82)
    {(mesa)\\[-3pt]Objective};

% ----------------------------------------------------------------
%  E) WORLD --- circle (shifted to 14.2)
% ----------------------------------------------------------------
\node[
    circle, draw, fill=white,
    ...
    align=center, font=\small\bfseries,
    line width=0.9pt
] (world) at (14.2, 2.10) {World};

% Dashed equator and meridian (updated to new x-coordinate)
\draw[dashed, line width=0.55pt]
    (14.2, 2.10) ellipse (0.90cm and 0.25cm);
\draw[dashed, line width=0.55pt]
    (14.2, 2.10) ellipse (0.25cm and 0.90cm);
...
mesa-optimizer, pursuing an internally learned mesa-objective when acting
in the world.
