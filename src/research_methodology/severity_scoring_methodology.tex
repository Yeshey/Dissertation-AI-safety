\begin{reviewed}
	...
	\]
	where $\bar{x}_{c,d}$ is the mean across the five raters for category $c$ on dimension $d$. All four dimensions enter with equal weight: no dimension is treated as a priori more important than the others, and no attempt is made to calibrate weights against the rating data. This keeps the composite score transparent and reproducible directly from Table~\ref{tab:rater_scores_summary}. The resulting score ranges from 1 to 5 on the same scale as the individual dimensions.
